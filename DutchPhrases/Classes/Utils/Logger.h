...
#define TRC_FRM(A) NSLog(@"FRAME: %s:%d:%@", __PRETTY_FUNCTION__,__LINE__, NSStringFromCGRect([A frame]));
#define TRC_BND(A) NSLog(@"BOUNDS: %s:%d:%@", __PRETTY_FUNCTION__,__LINE__, NSStringFromCGRect([A bounds]));
#else
#define TRC_DBG(A, ...)
#define TRC_FRM(A)
#define TRC_BND(A)
